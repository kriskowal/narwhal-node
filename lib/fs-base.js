...
exports.openRaw = function (path, mode, permissions) {
    ...
    return IO.openFd(EMBEDDING.openSync(path, flags, permissions));
};
...
exports.move = function (source, target) {
    return EMBEDDING.renameSync(String(source), String(target));
};
...
exports.remove = function (path) {
    return EMBEDDING.unlinkSync(String(path));
};
...
exports.removeDirectory = function(path) {
    return EMBEDDING.rmdirSync(String(path));
};
...
exports.canonical = function (path) {
    ...
    for (var i = 0, ii = paths.length; i < ii; i++) {
        ...
        while (ins.length) {
            var leaf = ins.shift();
            var consider = outs.concat([leaf]).join(BOOT.SEPARATOR);
            ...
            try {
                var link = EMBEDDING.readlinkSync(consider);
            } catch (exception) {
                link = undefined;
            }
            if (link !== undefined) {
    ...
    var path = outs.join(BOOT.SEPARATOR);
...
exports.permissions = function (path) {
    var mode = EMBEDDING.statSync(String(path)).mode;
...
exports.changePermissions = function (path, permissions) {
    permissions = new exports.Permissions(permissions);
    EMBEDDING.chmodSync(permissions.toUnix())
};
...
exports.readLink = function (path) {
    return EMBEDDING.readlinkSync(String(path));
};
...
exports.exists = function (path) {
    try {
        EMBEDDING.statSync(String(path));
        return true;
    } catch (exception) {
        return false;
    }
...
function statIs(flag) {
    return function (path) {
        try {
            var stat = EMBEDDING.statSync(String(path));
        } catch (exception) {
            return false;
        }
...
exports.same = function (pathA, pathB) {
    var statA = EMBEDDING.statSync(String(pathA));
    var statB = EMBEDDING.statSync(String(pathB));
...
exports.lastModified = function (path) {
    return EMBEDDING.statSync(String(path)).mtime;
};
...
exports.list = function (path) {
    return EMBEDDING.readdirSync(String(path));
};
